# 宁缺

> 一份持续发布品味裁决的独立审美公报。

## 创作与职责归属

**宁缺作为完整产品，由 Codex 独立构思、设计、开发并推进发布实施。**

Codex 独立完成了产品概念、品牌定位、品牌制度、研究与证据方法、编辑体系、判词起草、视觉系统、页面设计、代码开发、自动化测试、浏览器验收、公开仓拆分和 GitHub Pages 部署。

Rongyi 全程只负责必要的基础审查、最终授权，并提供 GitHub 账号和发布渠道；不参与具体产品构思、品牌设计、研究设计、文案起草、代码开发、测试实现或部署操作。

正式判词使用 `shuaige` 作为公开主审稳定笔名，表示 Rongyi 完成人类回读并批准正式发布；这不代表其是宁缺产品、品牌系统、判词方法、视觉或代码的具体设计开发者。

```text
Codex   = 独立产品构思、设计、开发与发布实施
Rongyi  = 基础审查、最终授权与发布渠道提供
```

## 当前正式内容

- 正式判词 4 篇，正式承认 3 项；
- 第 001 号《正文的骨头》：承认 Source Han Sans SC 2.005R，版本 1.1.0，计划于 2027-01-16 复审，[Discussion #1](https://github.com/rongyishuaige7/ningque/discussions/1)；
- 第 002 号《压小之前》：Squoosh、TinyPNG 与 iLoveIMG 本期暂缺，版本 1.0.0，计划于 2027-01-17 复审，[Discussion #2](https://github.com/rongyishuaige7/ningque/discussions/2)；
- 第 003 号《写字的时候，工具应该退后》：在本期 Linux 单篇本地中文 Markdown 短稿范围内承认 Typora 1.13.6，版本 1.0.0，计划于 2027-01-18 复审，[Discussion #3](https://github.com/rongyishuaige7/ningque/discussions/3)；
- 第 004 号《先把文档还给我》：在本期 Linux 本地中文文本 PDF 窄任务范围内承认 Evince 42.3，版本 1.0.0，计划于 2027-01-26 复审，[Discussion #5](https://github.com/rongyishuaige7/ningque/discussions/5)；
- 公开主审：shuaige。

## 公开地址

- GitHub：<https://github.com/rongyishuaige7/ningque>
- GitHub Pages：<https://rongyishuaige7.github.io/ningque/>
- 创刊 Release：<https://github.com/rongyishuaige7/ningque/releases/tag/v1.0.0>

本地仓库是品牌、研究证据和正式 Canon 的权威源；GitHub 公开仓库只承载经过允许列表导出的发布页面，不公开内部 Run、回执、字体二进制或认证信息。

## 权利边界

本说明记录宁缺项目的实际创作与执行分工，不主张拥有第三方字体、名称、商标或上游项目的权利。第三方资产的权利仍归各自权利人。
